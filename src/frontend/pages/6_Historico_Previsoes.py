...
st.caption("Registro de todas as previsoes realizadas via formulario de Previsao em Tempo Real")
...
if not previsoes:
    st.info("Nenhuma previsao registrada ainda. Acesse a pagina 'Previsao em Tempo Real' para gerar previsoes.")
    st.stop()
...
with c1:
    ...
    target_sel = st.multiselect("Filtrar por target", targets_disp, default=targets_disp)
with c2:
...
c2.metric("Targets distintos", df["target"].nunique())
...
for target in target_sel:
    ...
    if len(df_t) > 0:
        fig = px.line(
            ...
            x="ts",
            y="valor_previsto",
            title=f"Evolucao das previsoes — {target}",
            labels={"ts": "Timestamp", "valor_previsto": f"Valor previsto ({target})"},
            markers=True,
        )
...
st.dataframe(
    df_filt[["id", "ts_str", "cultura", "target", "valor_previsto", "modelo"]].rename(columns={
        "id": "ID",
        "ts_str": "Timestamp",
        "cultura": "Cultura",
        "target": "Target",
